...
- Keep the implementation aligned with the documented delivery order: stack done → typed local content model done → UI Phase 2 (Desktop + tablet landscape from Figma; next tablet portrait → mobile) → CMS → hardening.

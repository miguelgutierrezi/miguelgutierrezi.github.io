...
- **Phase 2 UI:** Desktop + **tablet landscape** (`max-width: 1279px`, Figma `cv-tablet-landscape`) one-pager baselines are in place. Tablet portrait and mobile pending. Product/architecture decisions live in `docs/` — consult before architectural changes.
...
- Delivery order for larger work: stack modernization (done) → typed local content model (done) → UI (Phase 2; **Desktop + tablet landscape done**, then tablet portrait → mobile via Figma MCP) → CMS → hardening.

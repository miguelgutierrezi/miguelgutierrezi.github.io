...
- **Phase 2 UI:** Desktop + tablet landscape/portrait + **mobile** one-pager baselines are in place. **Project detail** desktop + tablet landscape (`/projects/:id`, Figma `4:678` / `4:1170`) implemented; tablet portrait + mobile detail pending. Product/architecture decisions live in `docs/` — consult before architectural changes.
...
- One main page with anchored sections (`/#projects`, `#about`, `#experience`, `#courses`), not tabs that replace the visible content. Contact lives in the footer (`#contact`) and is **not** a nav/burger item. Dedicated project case studies use `/projects/:id` (desktop layout from Figma `project-detail-desktop`).
- Delivery order for larger work: stack modernization (done) → typed local content model (done) → UI (Phase 2; **Desktop + tablet L/P + mobile done**; **project-detail desktop done**, tablet/mobile detail next) → CMS → hardening.
